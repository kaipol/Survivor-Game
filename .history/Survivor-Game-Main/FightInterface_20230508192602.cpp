...
void FightInterface::paintMonsters() // 绘制怪物
{
    ...
    {
        pen.drawPixmap(Monsters_loc[i].first * 100, 100 + Monsters_loc[i].second * 70, 100, 70, *Monsters_pic[i]); // 绘制怪物
    }
}
...
void FightInterface::Heroes_and_Monsters_init(QString name)
{
    ...
    hero_hp_loc[0] = hero_loc[0] - 10, hero_hp_loc[1] = hero_loc[1] - 110;
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_loc.insert(i, QPair<int, int>(9, 9));              // 怪物位置初始化
