...
void FightInterface::keyPressEvent(QKeyEvent *event) // 键盘事件
{
    ...
    hero_hp_label->setGeometry(hero_hp_loc[0], hero_hp_loc[1], 100, 10); // 英雄血量显示位置
    update();
}
...
void FightInterface::Monsters_move() // 怪物移动函数
{
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_hp_label[i]->setGeometry(Monsters_hp_loc[i].first, Monsters_hp_loc[i].second, 100, 10); // 怪物血量显示位置
    }
    update();
}
...
void FightInterface::Heroes_and_Monsters_init(QString name) // 英雄和怪物初始化
{
    ...
    hero_hp_loc[0] = hero_loc[0] + 20, hero_hp_loc[1] = hero_loc[1] - 10;     // 英雄血量位置初始化
    hero_hp_label->setGeometry(hero_hp_loc[0], hero_hp_loc[1], 100, 10);      // 血量显示位置
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_hp_label[i]->setGeometry(Monsters_hp_loc[i].first + 20, Monsters_hp_loc[i].second - 20, 100, 10); // 怪物血量显示位置
        Monsters_hp_label[i]->setStyleSheet("font: bold 10px");                                                    // 设置字体
