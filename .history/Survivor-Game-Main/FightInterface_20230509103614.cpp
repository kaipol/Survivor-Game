...
FightInterface::FightInterface(QWidget *parent) : QWidget(parent)
{
    ...
    time_label->setStyleSheet("font: black 15px");                          // 设置字体
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        case 0: // 向上移动
            if (Monsters_loc[i].second > 0 && game_map[Monsters_loc[i].first / 100][Monsters_loc[i].second / 80 - 1] == 0)
            {
                ...
                {
                    if (i != j && Monsters_loc[i].first == Monsters_loc[j].first && Monsters_loc[i].second - 80 == Monsters_loc[j].second && Monsters_loc[j].first != hero_loc[0] && Monsters_loc[j].second != hero_loc[1])
                    {
                        loc_equal = true; // 位置相等
                        break;
                    }
        ...
        case 2: // 向左移动
            if (Monsters_loc[i].first > 0 && game_map[Monsters_loc[i].first / 100 - 1][Monsters_loc[i].second / 80] == 0)
            {
                for (int j = 0; j < MonstersNum; j++)
                {
                    if (i != j && Monsters_loc[i].first - 100 == Monsters_loc[j].first && Monsters_loc[i].second == Monsters_loc[j].second && Monsters_loc[j].first != hero_loc[0] && Monsters_loc[j].second != hero_loc[1])
                    {
                        loc_equal = true; // 位置相等
                        break;
                    }
        ...
        Monsters_hp_label[i]->setGeometry(Monsters_loc[i].first + 5, Monsters_loc[i].second - 30, 100, 10); // 怪物血量显示位置
    }
    update();
}
...
void FightInterface::paintEvent(QPaintEvent *event) // 绘制事件
{
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        pen.drawPixmap(Monsters_loc[i].first, Monsters_loc[i].second, 50, 40, *Monsters_pic[i]); // 绘制怪物
    }
}
...
void FightInterface::Heroes_and_Monsters_init(QString name) // 英雄和怪物初始化
{
    ...
    hero_hp_label->setStyleSheet("font: black 15px");                         // 设置字体
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_hp_label[i]->setGeometry(Monsters_loc[i].first, Monsters_loc[i].second - 50, 100, 10); // 怪物血量显示位置
        Monsters_hp_label[i]->setStyleSheet("font: black 15px");                                        // 设置字体
