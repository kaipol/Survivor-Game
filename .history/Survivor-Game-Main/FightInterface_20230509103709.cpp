...
FightInterface::FightInterface(QWidget *parent) : QWidget(parent)
{
    ...
    time_label->setStyleSheet("font: blod 15px");                           // 设置字体
...
void FightInterface::Monsters_move() // 怪物移动函数
{
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_hp_label[i]->setGeometry(Monsters_loc[i].first + 5, Monsters_loc[i].second - 50, 100, 10); // 怪物血量显示位置
    }
    update();
}
...
void FightInterface::Heroes_and_Monsters_init(QString name) // 英雄和怪物初始化
{
    ...
    hero_hp_label->setStyleSheet("font: blod 15px");                          // 设置字体
    ...
    for (int i = 0; i < MonstersNum; i++)
    {
        ...
        Monsters_hp_label[i]->setStyleSheet("font: blod 15px");                                         // 设置字体
