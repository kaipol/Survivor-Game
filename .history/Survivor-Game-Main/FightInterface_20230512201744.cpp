...
void FightInterface::bullet_move()
{
    ...
    switch (hero_direction)
    {
        {
        case 0: // 向上移动
            for (int i = 0; i < hero_bullets.size(); i++)
            {
                if (hero_bullets[i]->bullet_loc.second > 0 && game_map[hero_bullets[i]->bullet_loc.first / 100][hero_bullets[i]->bullet_loc.second / 80 - 1] == 0)
                {
        ...
        case 1: // 向下移动
            for (int i = 0; i < hero_bullets.size(); i++)
            {
                if (hero_bullets[i]->bullet_loc.second < 720 && game_map[hero_bullets[i]->bullet_loc.first / 100][hero_bullets[i]->bullet_loc.second / 80 + 1] == 0)
                {
                    hero_bullets[i]->bullet_loc.second += 80;
                }
                else
        ...
        case 2: // 向左移动
            for (int i = 0; i < hero_bullets.size(); i++)
            {
                if (hero_bullets[i]->bullet_loc.first > 0 && game_map[hero_bullets[i]->bullet_loc.first / 100 - 1][hero_bullets[i]->bullet_loc.second / 80] == 0)
                {
                    hero_bullets[i]->bullet_loc.first -= 100;
                }
                else
                {
                    hero_bullets.remove(i);
        ...
        case 3: // 向右移动
            for (int i = 0; i < hero_bullets.size(); i++)
            {
                if (hero_bullets[i]->bullet_loc.first < 900 && game_map[hero_bullets[i]->bullet_loc.first / 100 + 1][hero_bullets[i]->bullet_loc.second / 80] == 0)
                {
                    hero_bullets[i]->bullet_loc.first += 100;
                }
                else
...
void FightInterface::paint_bullet() // 绘制子弹
{
    ...
    for (int i = 0; i < hero_bullets.size(); i++)
    {
        pen.drawPixmap(hero_bullets[i]->bullet_loc.first, hero_bullets[i]->bullet_loc.second, 20, 20, *hero_bullets[i]->bullet_pic); // 绘制子弹
    }
}
