...
FightInterface::FightInterface(QWidget *parent) : QWidget(parent)
{
    ...
    remain_min = 0, remain_sec = 5;           // 剩余时间初始化
...
void FightInterface::get_map_type(int type)
{
    ...
    bullet_generate_timer->start(1200); // 子弹生成定时器开始
...
void FightInterface::Monsters_move()
{
    for (int i = 0; i < Monsters_all.size(); i++)
    {
        ...
        std::pair<int, int> pic_size = Monsters_all[i]->get_pic_size();

        // 计算怪物到英雄的距离
        int dx = hero_loc[0] - loc.first;
        int dy = hero_loc[1] - loc.second;

        // 根据怪物到英雄的距离计算下一个位置的QRect
        QRect next_rect;
        if (std::abs(dx) > std::abs(dy)) // 水平方向移动
        {
            if (dx > 0) // 英雄在右侧
            {
                next_rect = QRect(loc.first + speed, loc.second, pic_size.first, pic_size.second);
        ...
        else // 垂直方向移动
        {
            if (dy > 0) // 英雄在下方
            {
                next_rect = QRect(loc.first, loc.second + speed, pic_size.first, pic_size.second);
        ...
        {
            // 如果没有产生碰撞，则移动到下一个位置
            if (Monsters_all[i]->get_direction() == 0) // 向上移动
            {
                Monsters_all[i]->set_loc(loc.first, loc.second - speed);
            }
            else if (Monsters_all[i]->get_direction() == 1) // 向下移动
            {
                Monsters_all[i]->set_loc(loc.first, loc.second + speed);
            }
            else if (Monsters_all[i]->get_direction() == 2) // 向左移动
            {
                Monsters_all[i]->set_loc(loc.first - speed, loc.second);
            }
            else if (Monsters_all[i]->get_direction() == 3) // 向右移动
            {
                Monsters_all[i]->set_loc(loc.first + speed, loc.second);
            }
        }
...
void FightInterface::hero_level_up()
{
    ...
    if ((exp == 1 && level == 1) || (exp == 20 && level == 2) || (exp == 30 && level == 3) || (exp == 40 && level == 4))
    {
        level++;
