...
void FightInterface::keyPressEvent(QKeyEvent *event)
{
    ...
    for (int i = 0; i < drop_props.size(); i++)
    {
        ...
        if (prop_rect.intersects(hero_rect))
        {
            ...
            {
                int dir = hero->get_direction();
                QPainter painter(this);
                painter.drawPixmap(hero_loc[0], hero_loc[1], 40, 40, QPixmap("../src/Sprite/laser1.png"));
            }
            drop_props.remove(i);
            break;
        }
...
void FightInterface::drop_prop(int x, int y)
{
    ...
    else if (type < 110)
    {
        ...
        drop_prop_pic.push_back(QPixmap("../src/Sprite/laser.png"));
    }
}
