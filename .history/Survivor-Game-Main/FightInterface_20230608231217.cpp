...
void FightInterface::keyPressEvent(QKeyEvent *event)
{
    ...
    for (int i = 0; i < drop_props.size(); i++)
    {
        ...
        if (prop_rect.intersects(hero_rect))
        {
            ...
            else if (drop_props[i][2] == 3)
            {
                QRect hero_rect(hero_loc[0], hero_loc[1], 100, 100);
                for (int i = 0; i < Monsters_all.size(); i++)
                {
                    ...
                    {
                        Monsters_all[i]->set_blood(Monsters_all[i]->get_blood() - 10);
                        if (Monsters_all[i]->get_blood() <= 0)
                        {
                            Monsters_all.remove(i);
                            i--;
                            coin += base_kill_coin + Monsters_all[i]->get_monster_killed_coin();
                            coin_label->setText("金币数:" + QString::number(coin));
                            hero->set_hero_exp(hero->get_hero_exp() + base_kill_exp + Monsters_all[i]->get_monster_kill_exp());
                        }
                    }
                }
            }
...
void FightInterface::monsters_hit(int j, int attack)
{
    Monsters_all[j]->set_blood(Monsters_all[j]->get_blood() - attack);
    ...
    for (int i = 0; i < hero_bullets_loc.size(); i++)
    {
        ...
        else
        {
            for (int j = 0, k = Monsters_all.size() - 1; j <= k && i >= 0; j++, k--)
            {
                ...
                {
                    monsters_hit(j, hero->get_bullet_attack());
                    hero_bullets_loc.remove(i);
                    i--;
                }
...
void FightInterface::drop_prop(int x, int y)
{
    ...
    {
        drop_props.push_back({x, y, 3});
        drop_prop_pic.push_back(QPixmap("../src/Sprite/Bomb.jpg"));
    }
}
