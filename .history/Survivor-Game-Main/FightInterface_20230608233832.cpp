...
void FightInterface::keyPressEvent(QKeyEvent *event)
{
    ...
    for (int i = 0; i < drop_props.size(); i++)
    {
        ...
        if (prop_rect.intersects(hero_rect))
        {
            ...
            {
                QRect hero_rect(hero_loc[0] - 80, hero_loc[1] - 80, 200, 200);
...
bool FightInterface::monsters_hit(int j, int attack)
{
    ...
    if (Monsters_all[j]->get_blood() <= 0)
    {
        ...
        Monsters_all.erase(Monsters_all.begin() + j);
        return true;
    }
...
void FightInterface::bullet_move()
{
    ...
    for (int i = 0; i < hero_bullets_loc.size(); i++)
    {
        ...
        {
            for (int j = 0; j < Monsters_all.size() && i >= 0; j++)
            {
                ...
                {
                    if (monsters_hit(j, hero->get_bullet_attack()))
                        j--;
                    hero_bullets_loc.remove(i);
                    i--;
                }
...
void FightInterface::drop_prop(int x, int y)
{
    ...
    if (type < 20)
    {
        ...
        drop_prop_pic.push_back(QPixmap("../src/Sprite/hp.png"));
    }
    else if (type < 80)
    {
