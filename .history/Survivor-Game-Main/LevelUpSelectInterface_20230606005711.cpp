...
LevelUpSelectInterface::LevelUpSelectInterface(QWidget *parent) : QWidget(parent)
{
    ...
    select_three->setGeometry(720, 500, 150, 80); // 设置按钮位置和大小
...
void LevelUpSelectInterface::prop_generate()
{
    for (int i = 0; i < 3; i++)
    {
        while (!loc[i])
        {
            ...
            {
                label1->show();
                label1->setGeometry(70 + 350 * i, 300, 150, 80);
                label1_flag = true;
                loc[i] = 1;
            }
            ...
            {
                label2->show();
                label2->setGeometry(70 + 350 * i, 300, 150, 80);
                label2_flag = true;
                loc[i] = 2;
            }
            else if (type < 30 && !label2_flag)
            {
                label3->show();
                label3->setGeometry(70 + 350 * i, 300, 150, 80);
                label3_flag = true;
                loc[i] = 3;
            }
            else if (type < 40)
            {
                label4->show();
                label4->setGeometry(70 + 350 * i, 300, 150, 80);
                loc[i] = 4;
            }
            else if (type < 50)
            {
                label5->show();
                label5->setGeometry(70 + 350 * i, 300, 150, 80);
                loc[i] = 5;
            }
            else if (type < 60)
            {
                label6->show();
                label6->setGeometry(70 + 350 * i, 300, 150, 80);
                loc[i] = 6;
            }
        }
    }
