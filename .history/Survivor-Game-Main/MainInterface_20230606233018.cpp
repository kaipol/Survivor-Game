...
MainInterface::MainInterface(QWidget *parent) : QWidget(parent)
{
    ...
    startButton->setGeometry(430, 400, 150, 100); // 设置开始按钮位置
    load_button->setGeometry(430, 500, 150, 100); // 设置加载存档按钮位置
    exitButton->setGeometry(430, 600, 150, 100);  // 设置退出按钮位置
    continue_button->hide();                      // 隐藏继续游戏按钮
    enter_shop_button->hide();                    // 隐藏进入商店按钮
    ...
    bgm->setSource(QUrl::fromLocalFile("../src/bgm/MainBgm.mp3"));             // 设置背景音乐路径
    bgm->play();                                                               // 播放背景音乐
}
...
void MainInterface::is_started()
{
    ...
    startButton->setGeometry(430, 390, 150, 80); // 设置开始按钮位置
    continue_button->setGeometry(430, 470, 150, 80);
    load_button->setGeometry(430, 550, 150, 80);
    enter_shop_button->setGeometry(430, 630, 150, 80);
    exitButton->setGeometry(430, 710, 150, 80);
}

void MainInterface::again_game()
{
    continue_button->hide();                           // 隐藏继续游戏按钮
    startButton->setGeometry(430, 400, 150, 80);       // 设置开始按钮位置
    load_button->setGeometry(430, 480, 150, 80);       // 设置加载存档按钮位置
    enter_shop_button->setGeometry(430, 560, 150, 80); // 设置进入商店按钮位置
    exitButton->setGeometry(430, 640, 150, 80);        // 设置退出按钮位置
}
