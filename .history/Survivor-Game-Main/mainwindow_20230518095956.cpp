    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(fight_interface, &FightInterface::back_to_main, main_interface, &MainInterface::started_reload);                                // 返回主界面信号连接

    connect(fight_interface, &FightInterface::enter_shop_signal, shop_interface, &ShopInterface::get_coin);                          // 进入商店信号连接
    ...
    connect(load_interface, &LoadInterface::data_select_signal, fight_interface, &FightInterface::get_date);                         // 读取存档信号连接
